# 題目：產線感測器數據流處理與動態評分系統
背景：
我們從 SQL 抓出了一批機台的原始感測器 Log。這批資料非常混亂，包含斷訊產生的負值、讀取錯誤的空值，以及需要複雜計算的性能評分。
# 任務清單：
...
data = {
    ...
    'Efficiency': [0.9123, 0.8544, np.nan, 0.9567, 0.8812, 0.7034] # 包含空值與多位小數
}
